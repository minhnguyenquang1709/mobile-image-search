...
class ThumbnailWidget extends ConsumerStatefulWidget {
  ...
  final Function(MediaAsset mediaAsset)? onTap;

  const ThumbnailWidget({super.key, required this.mediaAsset, this.onTap});
...
class _ThumbnailWidgetState extends ConsumerState<ThumbnailWidget>
  ...
  Widget build(BuildContext context) {
    ...
    final isImageSelected = ref.watch(
      homeScreenSelectionControllerProvider.select(
        (state) => state.contains(widget.mediaAsset.assetId),
      ),
    );
    final assetId = widget.mediaAsset.assetId;
        ...
        return GestureDetector(
          onTap: () {
            final isInSelectionMode = ref
                .read(homeScreenSelectionControllerProvider)
                .isNotEmpty;
            if (isInSelectionMode) {
              isImageSelected
                  ? ref
                        .read(homeScreenSelectionControllerProvider.notifier)
                        .deselectMedia(mediaAsset)
                  : ref
                        .read(homeScreenSelectionControllerProvider.notifier)
                        .selectMedia(mediaAsset);
            } else {
              context.push(
          ...
          onLongPress: () {
            final selectionController = ref.read(
              homeScreenSelectionControllerProvider.notifier,
            );
            if (!isImageSelected) {
              selectionController.selectMedia(mediaAsset);
            }
          },
